...
// The Japanese carts (Pocket Monsters Aka/Midori/Ao/Pikachu) carry the same
// ASCII header titles ("POKEMON RED/GREEN/BLUE/YELLOW"), so the substring match
// recognizes the Japanese trio (Red/Green/Yellow) + Blue that PMS-J accepts.
...
//
// The Japanese Red/Green/Blue/Yellow saves use the SAME SRAM layout as the
// international games (verified against pret/pokered-jp sram.asm): the "main
// data" block runs 0x2598..0x3522 with a 1-byte complement checksum at 0x3523,
// the player name sits at 0x2598 (0x50-terminated) and the trainer ID at
// 0x2605. Only the in-game TEXT ENCODING differs — Japanese stores kana, which
// we decode to UTF-8 via kGen1JpCharmap (gen1_charmap_jp.h, generated from the
// JP charmap). There is no game-version field, so which game is shown comes
// from the cart header (gb_game_key), not the save.
struct SaveInfo {
    bool present = false;
    bool valid = false;  // Gen-1 main-data checksum matches
    std::string trainer; // decoded player name (UTF-8 kana)
...
SaveInfo read_gen1_save(const std::filesystem::path& save_path) {
    ...
    // Player name: up to 5 kana, 0x50-terminated, at 0x2598. Decode the JP
    // in-game text encoding to UTF-8 (rendered via the bundled Noto Sans JP).
    for (size_t i = 0x2598; i <= 0x25A2; ++i) {
        const uint8_t c = b[i];
        if (c == 0x50 || c == 0x00) break;
        const char* glyph = kGen1JpCharmap[c];
        if (glyph[0] != '\0') out.trainer += glyph;
    }
...
GameInfo info_for(const std::string& key) {
    // Japanese GB carts (PMS-J's Transfer Pak party import / GB Tower). Display
    // names are UTF-8 ("ポケットモンスター ..."), rendered with the bundled Noto
    // Sans JP subset. Green has no dedicated box art yet -> empty card (per the
    // launcher spec); Red/Yellow reuse the existing art, Blue uses blue.png.
    if (key == "yellow") return {"\xE3\x83\x9D\xE3\x82\xB1\xE3\x83\x83\xE3\x83\x88\xE3\x83\xA2\xE3\x83\xB3\xE3\x82\xB9\xE3\x82\xBF\xE3\x83\xBC\x20\xE3\x83\x94\xE3\x82\xAB\xE3\x83\x81\xE3\x83\xA5\xE3\x82\xA6", "carts/yellow.png"}; // ポケットモンスター ピカチュウ
    if (key == "red")    return {"\xE3\x83\x9D\xE3\x82\xB1\xE3\x83\x83\xE3\x83\x88\xE3\x83\xA2\xE3\x83\xB3\xE3\x82\xB9\xE3\x82\xBF\xE3\x83\xBC\x20\xE8\xB5\xA4", "carts/red.png"};      // ポケットモンスター 赤
    if (key == "green")  return {"\xE3\x83\x9D\xE3\x82\xB1\xE3\x83\x83\xE3\x83\x88\xE3\x83\xA2\xE3\x83\xB3\xE3\x82\xB9\xE3\x82\xBF\xE3\x83\xBC\x20\xE7\xB7\x91", "carts/empty.png"};    // ポケットモンスター 緑
    if (key == "blue")   return {"\xE3\x83\x9D\xE3\x82\xB1\xE3\x83\x83\xE3\x83\x88\xE3\x83\xA2\xE3\x83\xB3\xE3\x82\xB9\xE3\x82\xBF\xE3\x83\xBC\x20\xE9\x9D\x92", "carts/blue.png"};     // ポケットモンスター 青
...
void init_hook(RT64::RenderInterface* render_interface, RT64::RenderDevice* device) {
    ...
    const FontFace fonts[] = {
        ...
        {"NotoSansJP-Subset.ttf", true}, // fallback for JP kana/kanji (trainer names, cart labels)
